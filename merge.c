...
void copy_ind (individual *ind1, individual *ind2)
{
    ...
    if (nreal!=0)
    {
        for (i=0; i<nreal; i++)
        {
            ...
            ind2->job_time_start[i] = ind1->job_time_start[i];
            ind2->job_time[i] = ind1->job_time[i];
            ind2->job_limit[i] = ind1->job_limit[i];
            ind2->job_cost[i] = ind1->job_cost[i];
            ind2->predecessor_job_id[i] = ind1->predecessor_job_id[i];
            ind2->job_type[i] = ind1->job_type[i];      
    ...
    {
        ind2->resource_free_time[i] = ind1->resource_free_time[i];
    }    
